...
sub usage($)
{
	...
	print "If you intend to set the configuration to all product variants\n";
	print "then you need to select each variant diffconfig.\n";
	print "For e.g. \$ autoconfig.pl -t a:CONFIG_MAX_LIMIT=y ";
	print "<product> <product_j> <product_ets>\n\n";
	print "Refer files under arch/arm64/configs/diffconfig/ ";
	print "for all supported variant diffconfigs.\n";
	print "\n";
}
...
sub parse_task_description($)
{
	...
	if ($action eq "s") {
		...
		print "\nIf you intend to set the configuration to all product variants\n";
		print "then you need to select each variant diffconfig.\n";
		if ($task =~ /^a:(\w+)=(.+)/) {
			$key = $1;
			$value = $2;
