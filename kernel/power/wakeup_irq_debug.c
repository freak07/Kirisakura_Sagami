...
static unsigned int irqs_cpu_stat(unsigned int irq, int cpu)
{
...
static int update_irq_table(struct wakeup_irq *p, int irq)
{
	...
	list_for_each_entry(i, &p->head, list) {
		if (i->irq == irq) {
			i->irq_count = irqs_cpu_stat(i->irq, 0);
			return 1;
		}
	}
...
static void add_irq_to_table(struct wakeup_irq *p, int irq)
{
	...
	info->irq_count = irqs_cpu_stat(irq, 0);
	info->irq = irq;
...
static int wakeup_irq_resume(struct device *dev)
{
	...
	list_for_each_entry(i, &p->head, list) {
		...
		if (desc) {
			...
			diff = irqs_cpu_stat(i->irq, 0) - i->irq_count;
