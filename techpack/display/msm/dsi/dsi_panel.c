...
#if defined(CONFIG_DRM_SDE_SPECIFIC_PANEL) && (defined(CONFIG_MACH_SONY_PDX214) || defined(CONFIG_MACH_SONY_PDX214_J))
	if (panel->spec_pdata->aod_mode) {
		pr_notice("%s: AOD is on, skip timing switch cmd\n", __func__);
		mutex_unlock(&panel->panel_lock);
		return 0;
	}
#endif /* CONFIG_DRM_SDE_SPECIFIC_PANEL && (CONFIG_MACH_SONY_PDX214 || CONFIG_MACH_SONY_PDX214_J) */
