...
static int nfc_ese_hw_cold_reset(struct nfc_dev *nfc_dev)
{
	...
	mutex_lock(&nfc_dev->dev_ref_mutex);
	/* check if cold reset already in progress */
	if (cold_reset->in_progress) {
		pr_err("%s: cold reset already in progress\n", __func__);
		ret = -EBUSY;
		goto err;
	}
	if (gpio_is_valid(nfc_gpio->cld_rst)) {
		gpio_set_value(nfc_gpio->cld_rst, 1);
		usleep_range(ESE_HW_CLD_RST_HOLD_TIME_US,
				ESE_HW_CLD_RST_HOLD_TIME_US + 100);
		gpio_set_value(nfc_gpio->cld_rst, 0);
		ret = start_cold_reset_guard_timer(cold_reset);
		if (ret)
			pr_err("%s: error in mod_timer\n", __func__);
		else
			pr_info("%s: perform hardware cold reset successfully\n", __func__);
	} else {
		pr_err("%s: hardware cold reset gpio is invalid\n", __func__);
		ret = -EINVAL;
	}
err:
	mutex_unlock(&nfc_dev->dev_ref_mutex);
